...
from .base import STANDARDS_EN, DISCLAIMER_EN
...
<body>
<div class="container">
<div class="header"><h1>Website Accessibility Compliance Report</h1></div>
<div class="info">
<p>Page: {page_name}</p>
<p>URL: {page_url}</p>
...
'''
    for idx, violation in enumerate(violations, 1):
        ...
        for node in nodes:
            ...
            selectors.extend(target)
        unique_selectors = list(set(selectors)) if selectors else ['No element selector found']
...
<h3>{idx}. {violation['id']}【{violation['impact'].upper()}】</h3>
<p class="wcag">WCAG Level: {wcag}</p>
<p><strong>Description:</strong> {violation['description']}</p>
<p><strong>Recommendation:</strong> {violation['help']}</p>
<div class="selector-group">
<span class="selector-label">Affected Element Selectors:</span>
{''.join([f'<div class="selector-item">{sel}</div>' for sel in unique_selectors])}
</div>
</div>
'''
    html += f'''
